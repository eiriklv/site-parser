...
exports = module.exports = function(site, body, callback) {
    if (!body) return callback('no body to parse');
    if (!site) return callback('no template to parse body with');
...
function processElement(element, entry) {
    ...
    if (entry[element.name] && (element.type == 'url')) {
        entry[element.name] = helpers.fixRelativePath(entry[element.name], entry._origin);
    }
